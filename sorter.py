...
class Trans:  # класс створює словник відповідності символів латиниці та кирилиці
    ...
    latin_symbol = ("a", "b", "v", "g", "d", "e", "e", "j", "z", "i", "j", "k", "l", "m", "n", "o", "p", "r", "s", "t",
...
class Scan:  # сканування папки та запис файлів в списки відповідних типів
    JPEG_IMAGES = []
    JPG_IMAGES = []
    PNG_IMAGES = []
    SVG_IMAGES = []
    ...
    REGISTER_EXTENSION = {
        ...
        'DOC': DOC_DOCUMENTS, 'DOCX': DOCX_DOCUMENTS, 'TXT': TXT_DOCUMENTS, 'PDF': PDF_DOCUMENTS,
        'XLSX': XLSX_DOCUMENTS, 'PPTX': PPTX_DOCUMENTS,
...
class ReplaceFile(Normalize):  # перенесення файлів до папок які відповідають типу файлу
    ...
    def handle_folder(folder: Path):
        try:
            ...
            print(f"Не можливо видалити архів: {folder}")

    @input_error
    def replaceable_main(self):
        for file in Scan.JPEG_IMAGES:
            ReplaceFile.handle_pictures(file, self.folder / 'images' / 'JPEG')
        for file in Scan.JPG_IMAGES:
            ReplaceFile.handle_pictures(file, self.folder / 'images' / 'JPG')
        for file in Scan.PNG_IMAGES:
            ReplaceFile.handle_pictures(file, self.folder / 'images' / 'PNG')
        for file in Scan.SVG_IMAGES:
            ReplaceFile.handle_pictures(file, self.folder / 'images' / 'SVG')

        for file in Scan.MP3_AUDIO:
            ReplaceFile.handle_audio(file, self.folder / 'audio' / 'MP3')
        for file in Scan.OGG_AUDIO:
            ReplaceFile.handle_audio(file, self.folder / 'audio' / 'OGG')
        for file in Scan.WAV_AUDIO:
            ReplaceFile.handle_audio(file, self.folder / 'audio' / 'WAV')
        for file in Scan.AMR_AUDIO:
            ReplaceFile.handle_audio(file, self.folder / 'audio' / 'AMR')

        for file in Scan.AVI_VIDEO:
            ReplaceFile.handle_media(file, self.folder / 'video' / 'AVI')
        for file in Scan.MP4_VIDEO:
            ReplaceFile.handle_media(file, self.folder / 'video' / 'MP4')
        for file in Scan.MOV_VIDEO:
            ReplaceFile.handle_media(file, self.folder / 'video' / 'MOV')
        for file in Scan.MKV_VIDEO:
            ReplaceFile.handle_media(file, self.folder / 'video' / 'MKV')

        for file in Scan.DOC_DOCUMENTS:
            ReplaceFile.handle_documents(file, self.folder / 'documents' / 'DOC')

        for file in Scan.DOCX_DOCUMENTS:
            ReplaceFile.handle_documents(file, self.folder / 'documents' / 'DOCX')
        for file in Scan.TXT_DOCUMENTS:
            ReplaceFile.handle_documents(file, self.folder / 'documents' / 'TXT')
        for file in Scan.PDF_DOCUMENTS:
            ReplaceFile.handle_documents(file, self.folder / 'documents' / 'PDF')
        for file in Scan.XLSX_DOCUMENTS:
            ReplaceFile.handle_documents(file, self.folder / 'documents' / 'XLSX')
        for file in Scan.PPTX_DOCUMENTS:
            ReplaceFile.handle_documents(file, self.folder / 'documents' / 'PPTX')

        for file in Scan.MY_OTHER:
            ReplaceFile.handle_other(file, self.folder / 'MY_OTHER')

        for file in Scan.ZIP_ARCHIVES:
            ReplaceFile.handle_archive(file, self.folder / 'archives' / 'ZIP')
        for file in Scan.GZ_ARCHIVES:
            ReplaceFile.handle_archive(file, self.folder / 'archives' / 'GZ')
        for file in Scan.TAR_ARCHIVES:
            ReplaceFile.handle_archive(file, self.folder / 'archives' / 'TAR')

        for folder in Scan.FOLDERS[::-1]:
            ReplaceFile.handle_folder(folder)
...
class PrintResult(Scan):
    def print_result(self):
        result_lists = (self.JPEG_IMAGES, self.JPG_IMAGES, self.PNG_IMAGES, self.SVG_IMAGES,
                        ...
                        self.DOC_DOCUMENTS, self.DOCX_DOCUMENTS, self.TXT_DOCUMENTS, self.PDF_DOCUMENTS,
                        self.XLSX_DOCUMENTS, self.PPTX_DOCUMENTS,
                        self.MP3_AUDIO, self.OGG_AUDIO, self.WAV_AUDIO, self.AMR_AUDIO, self.ZIP_ARCHIVES,
                        self.GZ_ARCHIVES, self.TAR_ARCHIVES,
                        self.FOLDERS,
                        self.MY_OTHER)
...
class CleanFolderMain(PrintResult):  # меню користувача
    def run():
        ...
        while True:
            ...
            -- Введіть "0" - Для виходу в попереднє меню. \n\
            >>> ')
            if value == "1":
                ...
                while True:
                    ...
                    val = Path(input(F'\
              -- Вкажіть шлях до папки з файлами, яку потрібно відсортувати.\n\
            ...
            >>> '))
              ...
              -- Введіть назву папки в поточній директорії({current_folder}),\n\
                 Список папок в поточній директорії\n\
{current_dir}  \n \
              >>> '))
                        check = input(F'\
            ...
            >>> ')
                    ...
                    if check == "1":
                        ...
                        threads = []
                        th = Thread(target=r.replaceable_main())
                        th.start()
                        threads.append(th)
